        ...
        <div className="w-full max-w-screen-xl mx-auto
              ...
              <button key={tab.id} onClick={() => goTab(tab.id)}
                className={`relative flex items-center gap-1.5 px-2.5 xl:px-3 py-2 rounded-xl
                  ...
                  ${activeTab===tab.id
                    ...
                    : 'text-teal-700/70 hover:text-teal-900 hover:bg-white/60'}`}
              >
                {tab.icon}
                <span>{tab.label}</span>
                {tab.soon && (
                  <span className="absolute -top-1 -right-1 bg-yellow-400 text-teal-950
                    text-[6px] px-1 py-0.5 rounded-full font-black animate-bounce leading-none">
                    SOON
                  </span>
                )}
              </button>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {isLoggedIn ? (
              <div className="relative" ref={dropRef}>
                <button onClick={() => setDropdownOpen(p => !p)}
                  className="flex items-center gap-1.5 bg-teal-900 text-white
                    pl-2 pr-2.5 py-1.5 rounded-xl font-bold text-xs sm:text-sm
                    hover:bg-teal-800 transition-all shadow-md">
                  <span className="w-6 h-6 rounded-full bg-teal-500
                    flex items-center justify-center text-[10px] font-black uppercase shrink-0">
                    {userName ? userName[0].toUpperCase() : <User size={11}/>}
                  </span>
                  <span className="hidden sm:block max-w-[80px] xl:max-w-[110px] truncate">
                    {userName || "Usuario"}
                  </span>
              ...
              <Link to="/login"
                className="flex items-center gap-1.5 bg-teal-900 text-white
                  px-3 sm:px-4 py-2 rounded-xl font-bold text-xs sm:text-sm
                  hover:bg-teal-800 transition-all shadow-md">
                <User size={15}/>
            ...
            <button onClick={() => setMobileOpen(p => !p)}
              className="md:hidden p-2 rounded-xl bg-teal-900/10 hover:bg-teal-900/20
                text-teal-900 transition-colors"
      ...
      {mobileOpen && (
        <>
          ...
          <div className="fixed z-50 md:hidden
            top-[68px] left-3 right-3
            bg-white rounded-2xl shadow-2xl border border-teal-100 overflow-hidden">

            {tabs.map(tab => (
              <button key={tab.id} onClick={() => goTab(tab.id)}
                className={`w-full flex items-center gap-3 px-5 py-4
                  text-sm font-black uppercase tracking-wider
                  border-b border-teal-50 last:border-none transition-colors
                  ${activeTab===tab.id
              ...
              >
                ...
                <span>{tab.label}</span>
                {tab.soon && (
                  <span className="ml-auto bg-yellow-400 text-teal-950
                    text-[8px] px-2 py-0.5 rounded-full font-black">SOON</span>
                )}
              </button>
            ))}
              ...
              <>
                ...
                <Link to="/mis-reservas" onClick={() => setMobileOpen(false)}
                  className="flex items-center gap-3 px-5 py-4
                    text-sm font-semibold text-teal-700 hover:bg-teal-50 transition-colors">
