...
import { CreditCard, Wallet, Lock, CheckCircle, X, Sparkles, Loader2 } from 'lucide-react';
...
export function PostBookingModal({ isOpen, onClose, reservaId, precio, descripcion }: PostBookingModalProps) {
  ...
  const handleStripe = async () => {
    ...
    try {
      ...
      else setError(data.error || 'Error al iniciar Stripe');
    } catch { setError('Error de conexión con Stripe'); }
  ...
  const handlePaypal = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.url) window.location.href = data.url;
      else setError(data.error || 'Error al iniciar PayPal');
    } catch { setError('Error de conexión con PayPal'); }
    finally { setLoading(null); }
  };
        ...
        <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-md p-8 animate-in fade-in zoom-in duration-200">
          ...
          <div className="space-y-3">
            <button onClick={() => setShowPay(true)}
              className="w-full bg-teal-900 hover:bg-teal-800 text-white rounded-2xl py-4 font-black uppercase tracking-widest text-sm flex items-center justify-center gap-3 transition-all shadow-lg shadow-teal-900/20">
      ...
      <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-md p-8 animate-in fade-in zoom-in duration-200">
        ...
        <div className="space-y-3">
          <button onClick={handleStripe} disabled={loading !== null}
            className="w-full bg-[#635BFF] hover:bg-[#5147e6] text-white rounded-2xl py-4 font-black uppercase tracking-widest text-sm flex items-center justify-center gap-3 transition-all disabled:opacity-50 shadow-lg shadow-[#635BFF]/30">
            {loading === 'stripe'
          ...
          <button onClick={handlePaypal} disabled={loading !== null}
            className="w-full bg-[#FFC439] hover:bg-[#f0b72f] text-[#003087] rounded-2xl py-4 font-black uppercase tracking-widest text-sm flex items-center justify-center gap-3 transition-all disabled:opacity-50 shadow-lg shadow-[#FFC439]/30">
            {loading === 'paypal'
              ? <><Loader2 size={18} className="animate-spin"/>Redirigiendo...</>
