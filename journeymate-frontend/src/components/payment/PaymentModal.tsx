...
// Modal de pago que se usa desde MisReservas
// Recibe UNA reserva específica para pagar

import { useState } from 'react';
import { X, CreditCard, Wallet, Lock, CheckCircle, Loader2 } from 'lucide-react';
...
export function PaymentModal({ isOpen, onClose, reservaId, precio, descripcion }: PaymentModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-teal-950/70 backdrop-blur-md p-4">
      <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-md p-8 relative animate-in fade-in zoom-in duration-200">
        ...
        <button onClick={onClose} className="absolute top-6 right-6 text-teal-900/40 hover:text-teal-900 transition-colors">
          <X size={24} />
        </button>
        ...
        <div className="space-y-3">
          <button onClick={handleStripe} disabled={loading !== null}
            ...
            {loading === 'stripe' ? <><Loader2 size={18} className="animate-spin"/>Redirigiendo...</> : <><CreditCard size={20}/>Pagar con Stripe</>}
          </button>
          ...
          <button onClick={handlePaypal} disabled={loading !== null}
            ...
            {loading === 'paypal' ? <><Loader2 size={18} className="animate-spin"/>Redirigiendo...</> : <><Wallet size={20}/>Pagar con PayPal</>}
          </button>
        </div>
