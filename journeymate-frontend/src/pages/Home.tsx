...
// ── Categorías — cada una lleva a su tab ─────────────────
const CATEGORIES = [
...
// ── Destinos con aeropuerto IATA para prellenar vuelos ───
const DESTINOS = [
  { nombre: 'Tokio',      pais: 'Japón',     emoji: '🗼', tag: 'Tendencia', iata: 'TYO', destText: 'Tokio, Japón' },
  { nombre: 'Roma',       pais: 'Italia',    emoji: '🏛️', tag: 'Cultura',   iata: 'ROM', destText: 'Roma, Italia' },
  { nombre: 'Bali',       pais: 'Indonesia', emoji: '🌴', tag: 'Relax',     iata: 'DPS', destText: 'Bali, Indonesia' },
  { nombre: 'Nueva York', pais: 'EE.UU.',    emoji: '🗽', tag: 'Urbano',    iata: 'NYC', destText: 'Nueva York, EE.UU.' },
  { nombre: 'Dubái',      pais: 'EAU',       emoji: '🏙️', tag: 'Lujo',      iata: 'DXB', destText: 'Dubái, EAU' },
  { nombre: 'París',      pais: 'Francia',   emoji: '🗺️', tag: 'Romántico', iata: 'PAR', destText: 'París, Francia' },
];
...
export default function Home() {
  ...
  // Fix 1: cada categoría va a su tab
  const goToSearch = (section?: string) => {
    navigate(section ? `/buscar?tab=${section}` : '/buscar');
  };

  // Fix 2: destino → tab vuelos con destino preseleccionado
  ...
  return (
    ...
    <div className="min-h-screen font-sans overflow-x-hidden"
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 px-4 pt-3">
        <div className="max-w-6xl mx-auto bg-white/15 backdrop-blur-xl border border-white/20
          ...
          <div className="flex items-center gap-3">
            {userName ? (
              // ⭐ Usuario logueado: hola + mis reservas + dropdown logout
              <div className="flex items-center gap-2">
                ...
                <Link to="/mis-reservas"
                  className="flex items-center gap-1.5 text-white/80 hover:text-white
                    text-xs font-bold border border-white/20 px-3 py-2 rounded-xl
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16">
            <button onClick={() => goToSearch()}
              className="group flex items-center gap-3 bg-white text-teal-900 px-8 py-4
                ...
                hover:bg-teal-50 transition-all shadow-2xl shadow-black/20 hover:scale-105 active:scale-95">
              Empezar a explorar
              <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
            </button>
            {!userName && (
              <Link to="/login"
                className="flex items-center gap-2 text-white/80 hover:text-white font-bold text-sm
                  border border-white/20 px-6 py-4 rounded-2xl hover:border-white/40 hover:bg-white/10 transition-all backdrop-blur">
                Ya tengo cuenta
              </Link>
            )}
          </div>
      ...
      {/* ── CATEGORÍAS — Fix 1: cada una a su tab ───────── */}
      ...
      {/* ── DESTINOS — Fix 2: van a vuelos con destino ───── */}
      <section className="px-4 py-20">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {DESTINOS.map(d => (
              <button key={d.nombre} onClick={() => goToDestino(d)}
                ...
                <p className="text-white/50 text-[10px] font-medium mt-0.5">{d.pais}</p>
                <span className="inline-flex items-center gap-1 mt-2 bg-white/15 text-white/70 text-[9px]
      ...
      {/* ── FEATURES — Fix 3: fondo oscuro, texto visible ── */}
      <section className="px-4 py-20">
        <div className="max-w-6xl mx-auto">
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              <div key={i} className={`${f.bg} border backdrop-blur rounded-3xl p-7`}>
      ...
      {/* ── CTA FINAL — Fix 3: fondo oscuro asegurado ───── */}
