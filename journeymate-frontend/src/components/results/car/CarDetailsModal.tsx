...
export const CarDetailsModal = ({ isOpen, onClose, car, searchData }: CarDetailsModalProps) => {
  ...
  useEffect(() => {
    if (isOpen) {
      setIsBooked(false); setIsReserving(false); setLoginError('');
      setPostModalOpen(false); setReservaId(null);
  ...
  const formatPrice = (v: number) =>
    new Intl.NumberFormat('es-ES', { style: 'currency', currency: car.currency||'EUR', minimumFractionDigits: 2 }).format(v);
  ...
  const handleReserve = async () => {
    ...
    try {
      ...
      if (!response.ok) { setLoginError(await response.text() || 'Error al realizar la reserva'); return; }
    ...
    <>
      <div className="fixed inset-0 z-[100] flex items-start justify-center p-4 bg-teal-950/70 backdrop-blur-md overflow-y-auto">
        <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden my-8">
          ...
          <div className="bg-gradient-to-br from-teal-900 to-teal-700 p-8 text-white relative">
            <button onClick={onClose} className="absolute top-6 right-6 p-2 hover:bg-white/20 rounded-full transition-colors"><X size={20}/></button>
            <div className="flex items-center gap-4">
              <div className="bg-white/10 p-3 rounded-2xl border border-white/10"><Car size={24} className="text-teal-200"/></div>
              <div>
                <h2 className="text-2xl font-black uppercase tracking-tighter">Resumen del Alquiler</h2>
                <p className="text-teal-300 text-[10px] font-bold uppercase tracking-widest mt-1">Revisa los detalles antes de confirmar</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-8 space-y-6">
            <div className="flex items-center gap-6 bg-gray-50 rounded-[2rem] p-6">
              {car.imageUrl ? (
                <img src={car.imageUrl} alt={car.carName} className="h-24 w-36 object-contain mix-blend-multiply"/>
              ) : (
                <div className="h-24 w-36 bg-teal-100 rounded-2xl flex items-center justify-center"><Car size={40} className="text-teal-500"/></div>
              )}
              <div>
                <p className="text-[9px] font-black text-teal-500 uppercase tracking-widest mb-1">Vehículo</p>
                <h3 className="text-2xl font-black text-teal-900 uppercase tracking-tighter leading-none">{car.carName}</h3>
                <div className="flex items-center gap-2 mt-2"><Building2 size={12} className="text-teal-400"/><span className="text-xs font-bold text-teal-600">{car.vendorName}</span></div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-teal-50 rounded-2xl p-4">
                <div className="flex items-center gap-2 mb-1"><Calendar size={14} className="text-teal-500"/><p className="text-[8px] font-black text-teal-400 uppercase tracking-widest">Recogida</p></div>
                <p className="text-xs font-black text-teal-900">{formatDate(searchData?.startDate)}</p>
                <div className="flex items-center gap-1 mt-1"><Clock size={10} className="text-teal-400"/><p className="text-[9px] text-teal-600 font-bold">{searchData?.pickupTime||'10:00'}</p></div>
              </div>
              <div className="bg-teal-50 rounded-2xl p-4">
                <div className="flex items-center gap-2 mb-1"><Calendar size={14} className="text-teal-500"/><p className="text-[8px] font-black text-teal-400 uppercase tracking-widest">Devolución</p></div>
                <p className="text-xs font-black text-teal-900">{formatDate(searchData?.endDate)}</p>
                <div className="flex items-center gap-1 mt-1"><Clock size={10} className="text-teal-400"/><p className="text-[9px] text-teal-600 font-bold">{searchData?.dropoffTime||'10:00'}</p></div>
              </div>
            </div>
            ...
            <div className="bg-teal-950 rounded-[2rem] p-8 text-white">
              ...
              <div className="space-y-2 mb-6">
                <div className="flex justify-between text-[11px]"><span className="opacity-60 font-bold uppercase">Precio por día</span><span className="font-black">{formatPrice(car.price/days)}</span></div>
                <div className="flex justify-between text-[11px] border-t border-white/10 pt-2"><span className="opacity-60 font-bold uppercase">{days} {days===1?'día':'días'}</span><span className="font-black">{formatPrice(car.price)}</span></div>
              </div>
            ...
            {loginError&&<div className="bg-red-50 text-red-700 text-xs font-bold px-4 py-3 rounded-2xl border border-red-100 text-center">{loginError}{!localStorage.getItem('token')&&<div className="mt-2"><a href="/login" className="underline text-red-900">Iniciar sesión</a></div>}</div>}
          </div>

          {/* ACCIONES */}
          <div className="px-8 pb-8 flex gap-4">
            <button onClick={onClose} className="flex-1 py-4 text-gray-400 font-bold text-[11px] uppercase tracking-widest">Cancelar</button>
            <button onClick={handleReserve} disabled={isReserving||isBooked}
              className={`flex-[2] py-4 text-white text-[11px] font-black uppercase tracking-widest rounded-3xl shadow-xl transition-all flex items-center justify-center gap-2 ${isBooked?'bg-green-600 cursor-default':'bg-teal-600 hover:bg-teal-500 hover:-translate-y-0.5 active:scale-95 disabled:opacity-50'}`}>
              {isReserving?'Reservando...':(isBooked?<><CheckCircle2 size={16}/>Reservado</>:<><Car size={16}/>Confirmar Alquiler</>)}
            </button>
          </div>
        </div>
      </div>
      ...
      {reservaId&&(
        <PostBookingModal isOpen={postModalOpen} onClose={handleContinueShopping}
          reservaId={reservaId} precio={car.price}
          descripcion={`${car.carName} · ${days} ${days===1?'día':'días'}`}
        />
      )}
    </>
  );
