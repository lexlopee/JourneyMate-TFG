...
import {
  ...
  PackageOpen, AlertCircle, Clock, XCircle
} from "lucide-react";
...
export default function MisReservas() {
  ...
  // ── Cancelar reserva: cambia estado a CANCELADA en backend ──
  const handleCancel = async (idReserva: number) => {
    ...
    try {
      const res = await fetch(`http://localhost:8080/api/v1/reservas/${idReserva}/estado`, {
        method: "PATCH",
        headers: {
      ...
      // La reserva pasa a CANCELADA → ya no aparece en "Mis Reservas" (solo PENDIENTE)
                  ...
                  <div key={r.idReserva}
                    ...
                    <div className="p-5 flex items-start gap-4">
                      ...
                      <div className="shrink-0 flex flex-col items-end gap-2">
                        ...
                        {/* Confirmación inline en dos pasos */}
                        {confirmCancelId === r.idReserva ? (
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              ...
                              disabled={cancellingId === r.idReserva}
                              ...
                              {cancellingId === r.idReserva ? "..." : "Confirmar"}
                            </button>
                          </div>
                        ) : (
                          <button
                            onClick={() => setConfirmCancelId(r.idReserva)}
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-teal-900/50 text-[10px] font-black uppercase tracking-widest">
                      Total de {reservas.length} reserva{reservas.length !== 1 ? "s" : ""} pendientes
                    </p>
