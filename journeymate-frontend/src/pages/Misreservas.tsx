...
import {
  ...
  PackageOpen, AlertCircle, Clock, CreditCard
} from "lucide-react";
...
export default function MisReservas() {
  ...
  const [payReservaId,   setPayReservaId]   = useState<number>(0);
  const [payPrecio,      setPayPrecio]      = useState<number>(0);
  const [payDescripcion, setPayDescripcion] = useState<string>("");
  ...
  // ✅ Pagar todo de una vez — usa la primera reserva con el total acumulado
  const handlePayAll = () => {
    if (reservas.length === 0) return;
    setPayReservaId(reservas[0].idReserva);
    setPayPrecio(precioTotal);
    setPayDescripcion(
      `${reservas.length} reserva${reservas.length > 1 ? "s" : ""} — Total`
    );
  ...
  return (
    <>
      <div className="min-h-screen flex flex-col font-sans"
        ...
        <header className="sticky top-0 z-20 px-4 pt-4 pb-2">
          <div className="max-w-3xl mx-auto bg-white/70 backdrop-blur-md border border-white/50 rounded-2xl px-4 py-3 flex items-center justify-between shadow-lg">
            ...
            <span className="bg-teal-900 text-white text-xs font-black px-3 py-1 rounded-full">{reservas.length}</span>
          </div>
        </header>
            ...
            {reservas.length === 0 ? (
              <div className="flex flex-col items-center justify-center gap-4 mt-16">
                <div className="bg-white/60 backdrop-blur rounded-3xl p-10 flex flex-col items-center gap-4 shadow-xl">
                  ...
                  <p className="text-teal-900/60 font-black uppercase tracking-widest text-sm text-center">Aún no tienes reservas</p>
                  <Link to="/" className="bg-teal-900 text-white px-6 py-2 rounded-xl font-bold text-sm hover:bg-teal-800 transition-all">Explorar destinos</Link>
                </div>
              </div>
            ) : (
              <>
                <div className="space-y-4">
                  {reservas.map((r, i) => (
                    <div key={r.idReserva}
                      className="bg-white/75 backdrop-blur border border-white/60 rounded-3xl shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all duration-200"
                      ...
                      <div className="p-5 flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-teal-700/70 font-semibold">
                            ...
                            <span className="flex items-center gap-1"><Clock size={11}/>Reserva #{r.idReserva}</span>
                          </div>
                        </div>
                ...
                {/* ── RESUMEN TOTAL + BOTÓN PAGAR TODO ── */}
                <div className="mt-6 bg-white/80 backdrop-blur border border-white/60 rounded-3xl shadow-lg p-5">
                  <div className="flex items-center justify-between mb-5">
                    <div>
                      ...
                      <p className="text-teal-900 font-black text-3xl mt-0.5">{precioTotal.toFixed(2)} €</p>
                    </div>
                  ...
                  {/* ✅ UN SOLO BOTÓN — paga todo de una vez */}
                  <button
                  ...
                  >
                    <CreditCard size={20} />
                    Pagar todo — {precioTotal.toFixed(2)} €
                  </button>
                </div>
              </>
            )}
      ...
      <PaymentModal
        ...
        reservaId={payReservaId}
        precio={payPrecio}
        descripcion={payDescripcion}
      />
    </>
  );
}