...
export const FlightDetailsModal = ({ isOpen, onClose, details }: any) => {
  ...
  // --- SOLUCIÓN AL BUG: Resetear estados al abrir un nuevo vuelo ---
  useEffect(() => {
    if (isOpen) {
      setLoginError("");
      setSuccessMessage("");
      setIsReserving(false);
      setIsBooked(false); // Esto permite volver a reservar en otros vuelos
    }
  }, [isOpen, details?.data?.id]); // Se ejecuta al abrir o si cambia el ID del vuelo
  ...
  const handleReserve = async () => {
    ...
    try {
      ...
      const primerSegmento = data.segments?.[0]; 
      const segmentoRegreso = data.segments?.find((s: any, idx: number) => idx > 0); 
      ...
      // Limpiar mensaje tras 4 segundos pero mantener el botón bloqueado como "Reservado"
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-teal-950/60 backdrop-blur-md">
      ...
      {successMessage && (
        <div className="fixed top-6 right-6 bg-teal-600 text-white px-6 py-3 rounded-xl shadow-xl z-[999] animate-in slide-in-from-top-4 flex items-center gap-2">
      ...
      <div className="bg-white w-full max-w-3xl rounded-[3.5rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300 border border-white/20">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <div className="flex gap-2 mt-2">
                ...
                {data.travellers && (
                  <span className="text-[9px] bg-teal-500/30 px-2 py-1 rounded-md font-bold uppercase tracking-widest text-teal-100 flex items-center gap-1">
                    <Users size={10} /> {data.travellers.length} Pasajero{data.travellers.length > 1 ? 's' : ''}
                  </span>
                )}
              </div>
            </div>
                    ...
                    <div className="text-right flex flex-col items-end gap-1">
                        ...
                        {leg.flightInfo?.planeType && (
                          <span className="text-[8px] font-bold text-gray-500 uppercase tracking-widest">
                            Avión: {leg.flightInfo.planeType}
                          </span>
                        )}
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-3 mt-6">
                <div className="flex items-center gap-3 bg-teal-50/50 p-3 rounded-2xl border border-teal-100/20">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-black text-teal-900">
                      {segment.travellerCabinLuggage?.[0]?.luggageAllowance?.maxPiece > 0 
                        ? `${segment.travellerCabinLuggage[0].luggageAllowance.maxPiece} Pieza(s)` 
                        : 'No incluido'
                      }
                      {segment.travellerCabinLuggage?.[0]?.luggageAllowance?.maxWeightPerPiece > 0 && 
                ...
                <div className="flex items-center gap-3 bg-teal-50/50 p-3 rounded-2xl border border-teal-100/20">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-black text-teal-900">
                      {segment.travellerCheckedLuggage?.[0]?.luggageAllowance?.maxPiece > 0
                        ? `${segment.travellerCheckedLuggage[0].luggageAllowance.maxPiece} Pieza(s)`
                        : 'No incluido'
                      }
                      {segment.travellerCheckedLuggage?.[0]?.luggageAllowance?.maxWeightPerPiece > 0 && 
          ...
          {data.carbonEmissions?.footprintForOffer?.quantity && (
            <div className="mb-8 flex items-center justify-between bg-green-50 p-5 rounded-[2rem] border border-green-100">
              ...
              <p className="text-sm font-black text-green-700">
                {data.carbonEmissions.footprintForOffer.quantity} {data.carbonEmissions.footprintForOffer.unit || 'kg'}
              </p>
            </div>
          )}
            ...
            <div className="flex flex-col md:flex-row justify-between items-end gap-6 relative z-10">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center w-full min-w-[220px]">
                    ...
                    <span className="text-[11px] font-black">
                      {formatCurrency(data.priceBreakdown?.baseFare?.units || 0, data.priceBreakdown?.baseFare?.currencyCode)}
                    </span>
                  </div>
                  
                  {data.priceBreakdown?.fee?.units > 0 && (
                    <div className="flex justify-between items-center w-full border-t border-white/5 pt-2">
                      <span className="text-[11px] font-bold uppercase opacity-60">Cargos/Fees:</span>
                      <span className="text-[11px] font-black">
                        {formatCurrency(data.priceBreakdown.fee.units, data.priceBreakdown.fee.currencyCode)}
                      </span>
                    </div>
                  )}
                  ...
                  <div className="flex justify-between items-center w-full border-t border-white/5 pt-2">
                    ...
                    <span className="text-[11px] font-black">
                      {formatCurrency(data.priceBreakdown?.tax?.units || 0, data.priceBreakdown?.tax?.currencyCode)}
                    </span>
                  </div>

                  {data.priceBreakdown?.discount?.units > 0 && (
                    <div className="flex justify-between items-center w-full border-t border-white/5 pt-2 text-green-400">
                      <span className="text-[11px] font-bold uppercase opacity-80">Descuento:</span>
                      <span className="text-[11px] font-black">
                        -{formatCurrency(data.priceBreakdown.discount.units, data.priceBreakdown.discount.currencyCode)}
                      </span>
                    </div>
                  )}
                </div>
              ...
              <div className="text-right w-full md:w-auto mt-6 md:mt-0">
                ...
                <h4 className="text-6xl md:text-7xl font-black tracking-tighter leading-none">
                  {formatCurrency(data.priceBreakdown?.total?.units || 0, data.priceBreakdown?.total?.currencyCode)}
                </h4>
              </div>
            </div>
          </div>
        ...
        {loginError && (
          <div className="px-8 pt-6 pb-2 bg-white">
            <div className="bg-red-50 text-red-700 text-[11px] font-black uppercase tracking-widest px-6 py-4 rounded-2xl text-center border border-red-100">
              {loginError}
              {!isLogged && (
                <div className="mt-2">
                  <a href="/login" className="underline text-red-900 hover:text-red-500 transition-colors">
                    Iniciar sesión aquí
                  </a>
                </div>
              )}
        ...
        <div className="p-8 bg-white border-t border-gray-100 flex flex-col md:flex-row gap-4">
          <button 
            onClick={onClose} 
            className="flex-1 py-5 text-[11px] font-black uppercase tracking-widest text-gray-400 hover:text-teal-900 transition-all"
          >
            Cerrar
          </button>
          ...
          <button 
            ...
            className={`flex-[2] py-5 text-white text-[11px] font-black uppercase tracking-widest rounded-3xl shadow-xl transition-all flex items-center justify-center gap-3 ${
              isBooked 
                ? 'bg-green-600 cursor-default shadow-none opacity-90' 
                : 'bg-teal-600 hover:bg-teal-500 hover:-translate-y-1 active:scale-95 disabled:opacity-50'
            }`}
          >
            {isReserving ? (
              "Reservando Itinerario..."
            ) : isBooked ? (
              <>¡Reserva Confirmada! <CheckCircle2 size={16} /></>
            ) : (
              <>
                <span>Reservar Itinerario</span>
                {data.tripType === 'ROUNDTRIP' ? <ArrowLeftRight size={16} /> : <ArrowRight size={16} />}
              </>
            )}
          </button>
        </div>
